...
> ⚠️ **いま主戦場はこのリポジトリではありません。次に作るアプリを探している最中です。**
>
> **目標は月3万円。** 企画の判断基準・落とした案とその理由は
> [`docs/idea-log.md`](docs/idea-log.md) に集約してあります。**企画の話をする前に必ず読むこと。**
>
> 落とした案: 「このさき」（働き方比較）／「そだつ」（月齢カメラ）／アプリブロッカー
> 生きている候補: **保育園の連絡帳をAIが下書きする**（未検証）
>
> **禁止事項**: 5条件の「自分が続けられるか」を本人の確認なしに埋めないこと。
> 一度これをやって案を1つ無駄にしている（詳細は idea-log.md）。
>
> Life free 自体に手を入れるときは、この2つを読むこと。
> - [`docs/positioning.md`](docs/positioning.md) — 誰に何を売るのか
> - [`docs/app-concept.md`](docs/app-concept.md) — 何を作るのか（診断/計画/記録の3層）
> - 調査の進め方の手本: [`docs/sodatsu.md`](docs/sodatsu.md)（案は没だが手順は使える）
> - 成果物は `mock/` と `lp/` に残してある

**ターゲット**: **0〜7歳の子がいる親／これから子を持つ人**
**解く問題**: 「ぼんやりお金がかかるのは分かるが、何年後に何にいくらかかるか分からない」を、
具体的な時期と金額に変える
**一行**: 「一番きついのは、何年後か知っていますか」

**主役は予測。記録はその精度を上げる手段。** (現状のアプリは主従が逆になっている — positioning.md 参照)
