...
const MonthEditor: React.FC<MonthEditorProps> = ({ config, initialData, previousRecord, onSave, onCancel, currentTotalCash, isSaving = false }) => {
  ...
  const inputClass = "w-full bg-zinc-800 text-zinc-100 border border-zinc-700 rounded-lg p-2 focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition-colors text-sm";
  const labelClass = "block text-xs text-zinc-400 mb-1";

  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-start justify-center z-50 p-0 sm:p-4 overflow-y-auto">
      <div className="bg-zinc-950 border border-zinc-800 rounded-none sm:rounded-2xl shadow-2xl w-full h-full sm:h-auto sm:max-w-2xl sm:my-8 overflow-y-auto max-h-screen">
        {/* Header */}
        <div className="bg-zinc-950/95 backdrop-blur-lg border-b border-zinc-800 px-5 py-4 flex justify-between items-center text-zinc-100 sticky top-0 z-10">
          <h2 className="text-base sm:text-lg font-bold flex items-center gap-2">
            <Calculator size={18} className="text-blue-400" />
            {initialData ? '実績を編集' : '新しい月を記録'}
          </h2>
          <button onClick={onCancel} className="text-zinc-400 hover:text-zinc-200 transition-colors p-1 -mr-1">
            <X size={22} />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-3">
              <h3 className="text-xs font-semibold text-emerald-400 border-b border-zinc-800 pb-1.5">収入 +</h3>
              <div>
            ...
            <div className="space-y-3">
              <h3 className="text-xs font-semibold text-rose-400 border-b border-zinc-800 pb-1.5">支出 -</h3>
              <div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={handleInputChange}
                  className={`${inputClass} ${getNum(formData.pocketMoneyExpense) > config.pocketMoneyTarget ? 'border-rose-500/60 bg-rose-500/10 text-rose-200' : ''}`}
                />
              </div>
            </div>
          </div>
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4">
            <h3 className="font-semibold text-zinc-200 text-sm mb-3 flex items-center gap-2">
              <Wallet size={16} />
              資産残高の確定
            </h3>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        onChange={handleInputChange}
                        className={`${inputClass} pl-7 text-base font-semibold`}
                        placeholder="0"
                      />
                    </div>
                  </div>
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        onChange={handleInputChange}
                        className={`${inputClass} pl-7 text-base font-bold`}
                    ...
                    <div className="mt-1.5 flex justify-between text-[10px]">
                      ...
                      {investGap !== 0 && (
                        <span className={investGap > 0 ? 'text-emerald-400 font-medium' : 'text-rose-400 font-medium'}>
              ...
              <div className="bg-zinc-950/50 border border-zinc-800 rounded-lg p-3">
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <div className={`text-base font-semibold py-1.5 ${cashFlowResult >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
                  ...
                  <div>
                    ...
                    <div className="mt-1.5 flex justify-between text-[10px]">
                      ...
                      {cashGap !== 0 && (
                        <span className={cashGap > 0 ? 'text-emerald-400 font-medium' : 'text-rose-400 font-medium'}>
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4">
            <div className="flex items-center justify-between mb-2">
              ...
              <button
                ...
                disabled={isThinking}
                className="text-xs bg-zinc-800 border border-zinc-700 text-zinc-200 px-3 py-1.5 rounded-full hover:bg-zinc-700 disabled:opacity-50 transition-colors"
              >
                {isThinking ? '分析中...' : 'アドバイスをもらう'}
              </button>
            </div>
            {aiAdvice && (
              <div className="text-sm text-zinc-300 bg-zinc-950/60 p-3 rounded-lg border border-zinc-800 leading-relaxed mt-3">
                {aiAdvice}
              </div>
            )}
          </div>
          ...
          {/* Actions */}
          <div className="flex gap-3 pt-2 sticky bottom-0 bg-zinc-950 pb-2 -mx-5 px-5 border-t border-zinc-800/50">
            <button
              type="button"
              onClick={onCancel}
              disabled={isSaving}
              className="flex-1 px-4 py-2.5 rounded-xl text-zinc-300 bg-zinc-800 hover:bg-zinc-700 font-medium transition-colors disabled:opacity-50"
            >
              キャンセル
            </button>
            <button
              type="submit"
              disabled={isSaving}
              className="flex-1 px-4 py-2.5 rounded-xl bg-blue-600 text-white hover:bg-blue-500 font-medium shadow-lg shadow-blue-900/30 transition-all flex items-center justify-center gap-2 disabled:opacity-50"
            >
              {isSaving ? (
                <>
